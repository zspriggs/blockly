...
Your users likely won't want their blocks to vanish, but you can fix this by saving the state of the workspace and loading it back in the next time they visit the site. 
For this codelab, we'll save the state of the workspace to local browser storage.  
...
You'll also add a call to `load` outside of the change listener, so that your app finds the saved blocks on initial load.  
...
This change listener is not yet complete, since there are some types of events that you shouldn't save after. 
...
Since we're only saving the blocks, we won't save after these types of events. 
