...
Now add code to inject the Blockly editor in `src/index.js`. You'll first import Blockly, then set up each div from the `index.html`. 
...
Make sure both `toolbox.js` and `index.js` are saved, then refresh your `localhost` page. 
